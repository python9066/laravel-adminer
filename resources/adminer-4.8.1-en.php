...
', script("setupSubmitHighlight(document);");
					...
					if ($Ia) {
						...
						if (count($_POST) == 1 || DRIVER != $Vi || SERVER != $M || $_GET["username"] !== $V || DB != $l) adm_redirect(auth_url($Vi, $M, $V, $l));
...
', ($T == "PROCEDURE" ? "<td>" . html_select("fields[$t][inout]", explode("|", $Td), $o["inout"]) : ""), '<th>';
					...
					if (support("scheme")) {
						if (DB != "" && $_GET["ns"] !== "") {
							if (!isset($_GET["ns"])) adm_redirect(preg_replace('~ns=[^&]*&~', '', ME) . "ns=" . get_schema());
...
';
						...
						if ($_POST && !$n && !$_POST["add"] && !$_POST["drop_col"]) {
							...
							if (!$c) adm_redirect(ME . "table=" . urlencode($a));
...
';
						...
						if ($_POST && !$n && !isset($_POST["add_x"])) {
							...
							if ($_POST["drop"]) {
								...
								if (!$J["collation"]) adm_redirect(substr(ME, 0, -1));
